...
(function ($, document, window) {
	...
	function appendHTML() {
		...
		if (!$box) {
			...
			$box = $tag(div).attr({
				id: colorbox,
				// If a user still uses IE8 or lower, just let him go
			...
			// Add the cancel button on the bottom using absolute positioning
			$cancel = $('<a/>').attr({
				id: prefix + 'Cancel',
				"data-action": "cancel",
				"data-type": "button",
	...
	publicMethod.position = function (speed, loadedCallback) {
		...
		var loadedContentWidth = $loaded.outerWidth(true);
		...
		function modalDimensions() {
			...
			var height = (parseInt($box[0].style.height, 10) - interfaceHeight);
	...
	publicMethod.prep = function (object) {
		...
		function getHeight() {
			...
			if (settings.get('cancelButton') == 0 && settings.get('saveButton') == 0) {
				// var maxHeight = settings.get('maxHeight');
				// var initialHeight = settings.get('initialHeight');
